...
"""                                   
                                НЕДОСТАТКИ КЛАССА SINGELTON

Singelton - это одно неделимое глобальное состояние. Могут возникать проблемы с тестированием, расширением,
или детерминированности отдельных частей кода. Все компоненты, исп этот паттерн, оказываются жетско связанными.
Связанное зачастую приходится развязывать — например, чтобы написать тесты. И, тестировать код,
в котором очень часто используются сингелтоны, может стать проблемой."""